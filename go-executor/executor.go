...
func execute(req Req) Resp {
	...
	// Fetch the test input and expected solution for this problem from blob storage.
	...
	// COMPILE (with safety timeout, no stdin needed here)
	if lang.CompileCmd != nil {
	...
	// RUN (timeout handled INSIDE container), feeding the test input in via stdin
...
// compareOutput checks the actual output against the expected output line by
// line, trimming leading/trailing whitespace on each line before comparing.
// A trailing blank line at the end of either string is ignored so a stray
// trailing newline doesn't cause a false mismatch.
